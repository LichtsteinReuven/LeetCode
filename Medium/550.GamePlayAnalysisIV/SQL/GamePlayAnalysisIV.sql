...
-- PostgreSQL
WITH PlayersFirstLogin AS (
    SELECT DISTINCT ON (player_id) player_id, event_date
    FROM Activity
    ORDER BY player_id, event_date
)
SELECT ROUND(
             COUNT(CASE WHEN Activity.player_id = PlayersFirstLogin.player_id
             AND PlayersFirstLogin.event_date + INTERVAL '1 day' = Activity.event_date THEN 1 ELSE NULL END) * 1.0 /
             COUNT(DISTINCT PlayersFirstLogin.player_id), 2) AS fraction
FROM Activity, PlayersFirstLogin;
