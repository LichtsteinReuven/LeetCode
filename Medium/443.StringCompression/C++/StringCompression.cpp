...
public:
    int compress(vector<char>& chars)
    {
        ...
        for (; i < chars.size(); i++)
        {
            ...
            updateChars(chars, &returnSize, &currentCtr, i);
        }
    ...
    void updateChars(vector<char>& chars, int* returnSize, int* currentCtr, int i)
    {
        chars[(*returnSize)++] = chars[i - 1];
        stack<int> digits;
